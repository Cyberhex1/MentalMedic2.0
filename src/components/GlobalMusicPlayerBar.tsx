...
export const GlobalMusicPlayerBar: React.FC<GlobalMusicPlayerBarProps> = ({
  ...
  return (
    <div className="fixed bottom-3 left-1/2 -translate-x-1/2 w-[95%] max-w-4xl bg-slate-900/95 dark:bg-slate-950/95 border border-pink-500/30 backdrop-blur-xl text-white rounded-2xl p-3 md:px-5 md:py-2.5 shadow-2xl shadow-pink-500/20 z-40 animate-slideUp flex items-center justify-between gap-3">
      {/* Left: Track Info & Animated Wave */}
      <div className="flex items-center gap-3 min-w-0 max-w-[40%]">
        <div className="relative w-10 h-10 rounded-xl bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white shrink-0 shadow-md">
          {isPlaying ? (
        ...
        <div className="min-w-0">
          <h4 className="text-xs font-bold text-slate-100 truncate cursor-pointer hover:text-pink-400 transition-colors" onClick={onOpenMixer}>
            {title}
          </h4>
      ...
      {/* Right: Volume & Studio Modal Trigger */}
